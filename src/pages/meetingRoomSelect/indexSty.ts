...
export const RoomValBtn = styled(View)<{active:boolean,disable:boolean}>`
    ...
    color: ${props => props.active ? '#fff' : (props.disable ? '#C0C4CC' : '#303133')};
    width: 48px;
    height: 48px;
    border-radius: 50%;
    margin: 0 auto;
...
export const RoomTimeLine = styled(View)<{disable:boolean,type:string,isEnd:boolean,item:string,i:number}>`
  flex: auto;
  position: relative;
